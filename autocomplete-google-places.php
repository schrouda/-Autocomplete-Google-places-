...
define( 'AUTOCOMPLETE_GP_VERSION', '3.1.0' );

if ( ! defined( 'AUTOCOMPLETE_GP_ABSPATH_DIR' ) ) {
	define( 'AUTOCOMPLETE_GP_ABSPATH_DIR', plugin_dir_path( __FILE__ ) );
}

if ( ! defined( 'AUTOCOMPLETE_GP_ABSPATH_URL' ) ) {
	define( 'AUTOCOMPLETE_GP_ABSPATH_URL', plugin_dir_url( __FILE__ ) );
}

// Including the settings/admin file.
include 'admin_options.php';

// Including the front-end scripts.
...
function autocomplete_gp_google_scripts_enqueue() {
	...
	// Expose the country-scope configuration to the front-end script.
	$worldwide = autocomplete_gp_get_option( 'restrict_worldwide', '1' );
	$countries = autocomplete_gp_get_option( 'countries', array() );
	$countries = is_array( $countries ) ? array_values( $countries ) : array();
	$config    = array(
		'worldwide' => ( '1' === (string) $worldwide || empty( $countries ) ),
		'countries' => array_slice( $countries, 0, 5 ),
	);
	wp_add_inline_script( 'autocompletegp-script', 'window.AutocompleteGPConfig = ' . wp_json_encode( $config ) . ';', 'before' );
